    ...
    <div class="container">
        <div class="header  d-flex flex-column justify-content-between align-items-center mt-5">
            <div class="d-flex justify-content-between w-100 mb-3">
                <div>
                    <h1 class="h2 d-block mb-0 tiny-title">Geolocalização</h1>
                </div>
                <div class="flex-column align-content-center justify-content-center">
                    <a class="btn  btn-primary" href="{{ route('admin.geolocals.create') }}">+ Geolocalização</a>
                </div>
            </div>
        </div>
        <div class="ms-md-auto pe-md-3 d-flex align-items-center justify-content-end">
           <form action="{{ route('admin.geolocals.index') }}" method="GET" class="form">
               @csrf
               @method('GET')
...
{{--                    {{dd($museus)}}--}}
                        @if($geolocals->total())
                            @foreach($geolocals as $local)
                                <tr>
                                    <td class="">{{ $local->nome }}</td>
                                    <td class="d-flex justify-content-center gap-1">
                                        <div class="d-none d-md-table-cell">
                                            <a href="{{ route('admin.geolocals.edit', $local) }}" class="btn btn-primary m-0 btn-sm d-flex align-items-center gap-1">
                                                <span class="material-symbols-outlined">edit</span>
                                               <span>Editar</span>
                                            </a>
                                        </div>
                                        <div class="d-md-none d-table-cell">
                                            <a href="{{ route('admin.geolocals.edit', $local) }}" class="btn btn-primary m-0 btn-sm d-flex">
                                                <span class="material-symbols-outlined">edit</span>
                                            </a>
                                        </div>
                                        <form class="ms-2" action="{{ route('admin.geolocals.destroy', $local->id)}}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                        ...
                                        <div class="d-none d-md-table-cell">
                                            <a href="{{ route('admin.geolocals.show', $local->id) }}" class="btn btn-primary m-0 btn-sm btn_detalhes d-flex align-items-center gap-1" > <!-- data-index="" data-bs-toggle="modal" data-bs-target="#modalDetalhes" -->
                                                <span class="material-symbols-outlined">visibility</span>
                                                <span>Detalhes</span>
                                            </a>
                                        </div>
                                        <div class="d-md-none d-table-cell">
                                            <a href="{{ route('admin.geolocals.show', $local->id) }}" class="btn btn-primary m-0 btn_detalhes btn-sm d-flex align-items-center gap-1">
                                                <span class="material-symbols-outlined">visibility</span>
                                            </a>
                                        </div>
                                    </td>
